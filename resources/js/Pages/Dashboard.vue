...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <section class="mt-6 rounded-3xl border border-stone-200 bg-slate-950 p-6 text-white shadow-sm dark:border-slate-800"><div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-400">Investimentos · {{ selectedCurrency }}</p><h2 class="mt-2 text-xl font-bold">Posicao consolidada</h2><p class="mt-1 text-xs text-slate-400">Valores atuais sem conversao entre moedas.</p></div><Link :href="route('reports.index', { currency: selectedCurrency })" class="text-sm font-bold text-brand-400">Abrir relatorios</Link></div><div v-if="investment" class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><div v-for="item in [{ label: 'Custo em aberto', value: investment.cost }, { label: 'Valor atual', value: investment.current_value }, { label: 'Nao realizado', value: investment.market_return }, { label: 'Realizado', value: investment.realized_profit_loss }, { label: 'Proventos liquidos', value: investment.net_income }, { label: 'Retorno total', value: investment.total_return }]" :key="item.label" class="rounded-2xl bg-slate-900 p-4"><p class="text-xs text-slate-500">{{ item.label }}</p><p class="mt-2 text-lg font-bold">{{ formatMoney(item.value, selectedCurrency) }}</p></div></div><p v-else class="mt-6 text-sm text-slate-500">Nenhuma carteira nesta moeda.</p><p v-if="investment?.unpriced_holdings" class="mt-4 text-xs text-amber-400">{{ investment.unpriced_holdings }} posicao(oes) sem cotacao, avaliada(s) pelo custo medio.</p></section>
