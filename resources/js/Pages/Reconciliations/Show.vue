  ...
  <AuthenticatedLayout>
    ...
    <section class="mt-3 overflow-hidden rounded-3xl bg-slate-950 text-white shadow-xl">
      ...
      <div class="grid grid-cols-2 gap-px bg-slate-800 sm:grid-cols-4">
        <article class="bg-slate-950 p-5"><p class="text-xs text-slate-500">Saldo declarado</p><p class="mt-1 text-2xl font-bold">{{ formatMoney(reconciliation.declared_balance, reconciliation.currency) }}</p></article>
        <article class="bg-slate-950 p-5"><p class="text-xs text-slate-500">Saldo detectado</p><p class="mt-1 text-2xl font-bold">{{ formatMoney(reconciliation.detected_balance, reconciliation.currency) }}</p></article>
        <article class="bg-slate-950 p-5"><p class="text-xs text-slate-500">Diferenca</p><p class="mt-1 text-2xl font-bold" :class="balanceDeltaTone">{{ formatMoney(reconciliation.delta, reconciliation.currency) }}</p></article>
        <article class="bg-slate-950 p-5"><p class="text-xs text-slate-500">Linhas</p><p class="mt-1 text-2xl font-bold">{{ reconciliation.summary.total }}</p></article>
      </div>
    </section>
    ...
    <section class="mt-6 overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="bg-stone-50 text-xs text-stone-500 dark:bg-slate-950"><tr><th class="px-4 py-3">Linha</th><th class="px-4 py-3">Data</th><th class="px-4 py-3 text-right">Valor</th><th class="px-4 py-3">Descricao</th><th class="px-4 py-3">Regra</th><th class="px-4 py-3">Situacao</th></tr></thead><tbody class="divide-y divide-stone-100 dark:divide-slate-800"><tr v-for="row in rows" :key="row.id"><td class="px-4 py-3 font-mono text-xs text-stone-400">{{ row.row_number }}</td><td class="px-4 py-3">{{ row.date ? formatDate(row.date) : '—' }}</td><td class="px-4 py-3 text-right font-semibold">{{ formatMoney(row.amount, reconciliation.currency) }}</td><td class="max-w-64 truncate px-4 py-3 text-stone-500">{{ row.description || '—' }}</td><td class="px-4 py-3 text-xs text-stone-400">{{ ruleLabel(row.match_rule) }}</td><td class="min-w-56 px-4 py-3"><span class="rounded-full px-2 py-1 text-[11px] font-semibold" :class="statusClassFor(row)">{{ statusLabelFor(row) }}</span><p v-if="row.errors.length" class="mt-2 text-xs text-rose-600">{{ row.errors.join(' ') }}</p></td></tr></tbody></table></div>
    </section>
