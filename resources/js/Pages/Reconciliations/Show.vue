...
const summaryCards = computed(() => [
    ...
    { label: 'Não encontradas', value: props.reconciliation.summary.missing, tone: 'text-amber-600' },
    { label: 'Não declaradas', value: props.reconciliation.summary.extras, tone: 'text-rose-600' },
    { label: 'Inválidas', value: props.reconciliation.summary.invalid, tone: 'text-stone-400' },
]);
...
const statusLabelFor = (row: Row) => ({ matched: 'Conciliada', missing: 'Não encontrada', invalid: 'Inválida' }[row.status] ?? row.status);
...
const typeLabel = (type: string) => ({ income: 'Receita', expense: 'Despesa', transfer_in: 'Transferência (entrada)', transfer_out: 'Transferência (saída)' }[type] ?? type);
...
const discard = () => {
    if (window.confirm('Descartar esta prévia?')) {
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <section class="mt-3 overflow-hidden rounded-3xl bg-slate-950 text-white shadow-xl">
      <div class="flex flex-col gap-5 border-b border-slate-800 p-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p class="mt-1 text-xs text-slate-400">{{ formatDate(reconciliation.period_start) }} até {{ formatDate(reconciliation.statement_date) }} · {{ reconciliation.currency }}</p>
        </div>
      ...
      <div class="grid grid-cols-2 gap-px bg-slate-800 sm:grid-cols-4">
        ...
        <article class="min-w-0 bg-slate-950 p-4 sm:p-5"><p class="text-xs text-slate-500">Diferença</p><p class="mt-1 truncate text-lg font-bold sm:text-2xl" :class="balanceDeltaTone">{{ formatMoney(reconciliation.delta, reconciliation.currency) }}</p></article>
    ...
    <section v-if="extras.length" class="mt-6 overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <header class="border-b border-stone-100 p-5 dark:border-slate-800"><h2 class="font-semibold">Lançamentos não declarados no extrato</h2><p class="text-xs text-stone-500">Movimentações do período sem nenhuma linha correspondente. Exibindo até 100.</p></header>
      <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="bg-stone-50 text-xs text-stone-500 dark:bg-slate-950"><tr><th class="px-4 py-3">Data</th><th class="px-4 py-3 text-right">Valor</th><th class="px-4 py-3">Tipo</th><th class="px-4 py-3">Descrição</th></tr></thead><tbody class="divide-y divide-stone-100 dark:divide-slate-800"><tr v-for="extra in extras" :key="extra.id"><td class="px-4 py-3">{{ formatDate(extra.date) }}</td><td class="px-4 py-3 text-right font-semibold" :class="extra.type === 'expense' || extra.type === 'transfer_out' ? 'text-rose-600' : 'text-emerald-600'">{{ sign(extra.type) }}{{ formatMoney(extra.amount, reconciliation.currency) }}</td><td class="px-4 py-3 text-xs">{{ typeLabel(extra.type) }}</td><td class="max-w-72 truncate px-4 py-3 text-stone-500">{{ extra.description || '—' }}</td></tr></tbody></table></div>
    </section>
  </AuthenticatedLayout>
</template>