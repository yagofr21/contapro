...
<template>
  <Head title="Conciliações" />
  <AuthenticatedLayout>
    <PageHeader kicker="Conferência de extrato" title="Conciliações" subtitle="Compare o saldo declarado pelo banco com o saldo detectado nos seus lançamentos, linha a linha.">
      <template #actions>
    ...
    <section class="mt-7 grid gap-4 lg:grid-cols-3">
      <article v-for="(step, index) in [{ title: 'Enviar', text: 'Extrato CSV com Data, Descrição e Valor' }, { title: 'Conferir', text: 'Regras de casamento com seus lançamentos' }, { title: 'Confirmar', text: 'Situação Reconciliada ou Divergente' }]" :key="step.title" class="rounded-2xl border border-stone-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
    ...
    <section class="mt-6 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      ...
      <form class="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_1fr_1fr_1.6fr_auto] lg:items-end" @submit.prevent="submit">
        ...
        <button :disabled="form.processing" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 disabled:opacity-50"><FileSpreadsheet :size="17" />Gerar prévia</button>
      </form>
    </section>
