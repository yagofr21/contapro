...
import { ArrowDownLeft, CalendarClock, CalendarDays, History, Plus, Repeat } from '@lucide/vue';
...
const props = defineProps<{ events: AgendaEvent[]; projection: Projection[]; horizonDays: number }>();
...
<template>
  ...
  <AuthenticatedLayout>
    <section class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
      <div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600">Agenda</p><h1 class="mt-2 text-3xl font-bold tracking-tight">Agenda e projeção</h1><p class="mt-2 text-sm text-stone-500 dark:text-slate-400">Próximos {{ horizonDays }} dias de recorrências, parcelas e receitas futuras pendentes.</p></div>
      <div class="flex gap-2">
        <Link :href="route('recurring.create')" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 hover:bg-brand-700"><Repeat :size="18" />Recorrência</Link>
    ...
    <section v-if="projection.length" class="mt-8 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
      <article v-for="account in projection" :key="account.id" class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <p class="mt-1 text-xs text-stone-400">saldo atual {{ formatMoney(account.balance) }} ao fim dos {{ horizonDays }} dias</p>
      </article>
    </section>

    <section v-if="groups.length" class="mt-8">
      <h2 class="text-lg font-bold">Lançamentos agendados</h2>
        ...
        <ul class="divide-y divide-stone-100 dark:divide-slate-800">
          <li v-for="(event, index) in dayEvents" :key="index" class="flex items-center justify-between gap-3 px-5 py-3">
            <div class="flex items-center gap-3">
              <span class="grid h-8 w-8 place-items-center rounded-xl" :class="eventTone(event.kind)"><component :is="eventIcon(event.kind)" :size="15" /></span>
              <div>
                <p class="text-sm font-semibold">{{ event.description ?? 'Sem descrição' }}</p>
                <p class="text-xs text-stone-400">{{ eventKindLabel(event.kind) }} · {{ event.account_name }}{{ event.category_name ? ' · ' + event.category_name : '' }}</p>
              </div>
            </div>
