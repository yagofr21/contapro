...
<template>
  <Head title="Ativos" />
  <AuthenticatedLayout>
    <PageHeader kicker="Catálogo de mercado" title="Ativos" subtitle="Escolha uma carteira e registre sua compra com quantidade, preço e taxas.">
      <template #actions>
        <Link :href="route('assets.create')" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white"><Plus :size="18" />Cadastrar no catálogo</Link>
      </template>
    </PageHeader>
    ...
    <div class="mt-6 flex gap-3 rounded-2xl border border-brand-100 bg-brand-50/60 p-4 text-sm text-brand-900 dark:border-brand-900 dark:bg-brand-950/30 dark:text-brand-100">
      ...
      <p><strong>Catálogo não é posição.</strong> A quantidade nasce ao registrar uma compra em uma carteira. Use o atalho de cada ativo abaixo.</p>
    </div>
    ...
    <EmptyState v-else-if="assets.length" class="mt-6" :icon="Search" title="Nenhum ativo encontrado" description="Ajuste o termo de busca ou cadastre o ativo no catálogo." />
    <EmptyState v-else class="mt-8" :icon="CandlestickChart" title="Nenhum ativo no catálogo" description="Cadastre o primeiro ativo para depois registrar uma compra." />
  </AuthenticatedLayout>
</template>
