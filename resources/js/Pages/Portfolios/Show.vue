...
const removeOperation = (transaction: InvestmentTransaction) => {
    if (confirm('Remover esta operação e recalcular todos os resultados posteriores?')) {
...
<template>
  ...
  <AuthenticatedLayout>
    <PageHeader :kicker="`Carteira · ${portfolio.currency}`" :title="portfolio.name" subtitle="Posição e resultados reconstruídos a partir do histórico completo.">
      <template #title-suffix>
        <Link :href="route('portfolios.edit', portfolio.id)" class="rounded-lg p-2 text-stone-400 hover:bg-stone-100 hover:text-brand-600 dark:hover:bg-slate-800" :aria-label="`Editar carteira ${portfolio.name}`"><Pencil :size="17" /></Link>
      </template>
      <template #actions>
        ...
        <Link :href="route('investment-transactions.create', portfolio.id)" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-600/20"><Plus :size="18" />Nova operação</Link>
      </template>
    </PageHeader>
    ...
    <section class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <article v-for="item in [
        ...
        { label: 'Resultado não realizado', value: summary.market_return, result: true },
    ...
    <Card title="Posições" subtitle="Quantidade e custo médio atuais" class="mt-6">
        ...
        <div class="md:hidden divide-y divide-stone-100 dark:divide-slate-800">
          <article v-for="holding in holdings" :key="holding.id" class="flex items-center gap-3 px-5 py-4">
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-0.5 text-xs text-stone-400">{{ formatDecimal(holding.quantity) }} un. · Custo médio {{ formatMoney(holding.average_cost, holding.currency) }}</p>
              <p class="mt-0.5 text-xs text-stone-400">Preço atual {{ formatMoney(holding.current_price, holding.currency) }}</p>
            </div>
