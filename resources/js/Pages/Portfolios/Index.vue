...
const remove = (portfolio: PortfolioSummary) => {
    if (confirm(`Remover a carteira "${portfolio.name}" e todo o histórico dela?`)) router.delete(route('portfolios.destroy', portfolio.id));
};
</script>

<template>
  <Head title="Carteiras" /><AuthenticatedLayout>
    <PageHeader kicker="Patrimônio" title="Carteiras de investimentos" subtitle="Posições, custo médio e resultado em um único retrato.">
      <template #actions>
