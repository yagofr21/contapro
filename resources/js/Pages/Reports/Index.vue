...
type Summary = { income: string; expenses: string; net: string; transaction_count: number; net_investment_income: string };
type Monthly = { month: string; income: string; expenses: string };
type Category = { name: string; color: string; total: string };
type InvestmentSummary = { current_value: string; market_return: string; unpriced_holdings: number };
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <section class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><article v-for="item in [{ label: 'Receitas', value: summary.income, tone: 'text-emerald-600' }, { label: 'Despesas', value: summary.expenses, tone: 'text-rose-600' }, { label: 'Fluxo liquido', value: summary.net, tone: Number(summary.net) >= 0 ? 'text-brand-600' : 'text-rose-600' }, { label: 'Proventos', value: summary.net_investment_income, tone: 'text-amber-600' }]" :key="item.label" class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><p class="text-xs text-stone-400">{{ item.label }}</p><p class="mt-2 text-xl font-bold" :class="item.tone">{{ formatMoney(item.value, filters.currency) }}</p></article><article class="rounded-2xl bg-slate-950 p-5 text-white"><p class="text-xs text-slate-500">Lancamentos</p><p class="mt-2 text-xl font-bold">{{ summary.transaction_count }}</p></article></section>

    <section class="mt-6 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><header><h2 class="font-semibold">Fluxo mensal</h2><p class="text-xs text-stone-400">Receitas e despesas no periodo</p></header><VChart class="mt-4 h-80" :option="cashFlowOption" autoresize /></section>

    <section class="mt-6 grid gap-6 xl:grid-cols-2"><article class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><header><h2 class="font-semibold">Despesas por categoria</h2><p class="text-xs text-stone-400">Distribuicao do periodo</p></header><VChart v-if="categories.length" class="h-72" :option="categoryOption" autoresize /><div v-else class="grid h-72 place-items-center text-sm text-stone-400">Sem despesas no periodo.</div></article><article class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><header><h2 class="font-semibold">Alocacao atual</h2><p class="text-xs text-stone-400">Posicao por tipo de ativo em {{ filters.currency }}</p></header><VChart v-if="allocation.length" class="h-72" :option="allocationOption" autoresize /><div v-else class="grid h-72 place-items-center text-sm text-stone-400">Sem investimentos nesta moeda.</div><p v-if="investmentSummary?.unpriced_holdings" class="mt-2 text-xs text-amber-600">{{ investmentSummary.unpriced_holdings }} posicao(oes) sem cotacao atual.</p></article></section>

    <section v-if="investmentSummary" class="mt-6 flex flex-col gap-4 rounded-3xl bg-slate-950 p-6 text-white sm:flex-row sm:items-center sm:justify-between"><div><p class="text-xs uppercase tracking-[0.18em] text-brand-400">Posicao atual</p><p class="mt-2 text-2xl font-bold">{{ formatMoney(investmentSummary.current_value, filters.currency) }}</p></div><div class="sm:text-right"><p class="text-xs text-slate-500">Resultado de mercado</p><p class="mt-1 text-lg font-bold" :class="Number(investmentSummary.market_return) >= 0 ? 'text-emerald-400' : 'text-rose-400'"><TrendingUp :size="17" class="mr-1 inline" />{{ formatMoney(investmentSummary.market_return, filters.currency) }}</p></div></section>
  </AuthenticatedLayout>
</template>
