...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <section class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6"><article v-for="item in [{ label: 'Receitas', value: summary.income, tone: 'text-emerald-600' }, { label: 'Despesas', value: summary.expenses, tone: 'text-rose-600' }, { label: 'Fluxo líquido', value: summary.net, tone: Number(summary.net) >= 0 ? 'text-brand-600' : 'text-rose-600' }, { label: 'Proventos', value: summary.net_investment_income, tone: 'text-amber-600' }, { label: 'Resultado realizado', value: summary.realized_profit_loss, tone: Number(summary.realized_profit_loss) >= 0 ? 'text-emerald-600' : 'text-rose-600' }]" :key="item.label" class="min-w-0 rounded-2xl border border-stone-200 bg-white p-3 shadow-sm sm:p-4 dark:border-slate-800 dark:bg-slate-900"><p class="truncate text-[11px] text-stone-400">{{ item.label }}</p><p class="mt-1 truncate text-base font-bold sm:text-lg" :class="item.tone">{{ formatMoney(item.value, filters.currency) }}</p></article><article class="min-w-0 rounded-2xl bg-slate-950 p-3 text-white sm:p-4"><p class="text-[11px] text-slate-500">Lançamentos</p><p class="mt-1 text-base font-bold sm:text-lg">{{ summary.transaction_count }}</p></article></section>
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-2">
      <Card title="Despesas por categoria" subtitle="Distribuição do período"><div class="p-4 sm:p-5"><div v-if="sortedCategories.length"><VChart class="h-48 sm:h-56" :option="categoryOption" autoresize /><ul class="mt-3 space-y-2.5 border-t border-stone-100 pt-3 dark:border-slate-800"><li v-for="category in sortedCategories" :key="category.name" class="flex items-center gap-2.5"><span class="h-2.5 w-2.5 shrink-0 rounded-full" :style="{ backgroundColor: category.color }" /><span class="min-w-0 flex-1 truncate text-sm text-stone-600 dark:text-slate-300">{{ category.name }}</span><span class="whitespace-nowrap text-sm font-bold">{{ formatMoney(category.total, filters.currency) }}</span></li></ul></div><EmptyState v-else title="Sem despesas no período." /></div></Card>
    ...
    <section v-if="investmentSummary" class="mt-6 grid grid-cols-2 gap-3 rounded-3xl bg-slate-950 p-5 text-white sm:gap-4 sm:p-6 lg:grid-cols-4">
      ...
      <div><p class="text-xs text-slate-500">Não realizado</p><p class="mt-1 text-base font-bold sm:text-lg" :class="Number(investmentSummary.market_return) >= 0 ? 'text-emerald-400' : 'text-rose-400'">{{ formatMoney(investmentSummary.market_return, filters.currency) }}</p></div>
