...
const form = useForm({
    ...
    limit_amount: formatDecimal(props.budget?.limit_amount ?? '', 2, 4),
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        limit_amount: parseDecimalInput(data.limit_amount),
        ends_on: isCustom.value ? data.ends_on : null,
    }));
...
<template>
  <form class="mt-8 max-w-2xl rounded-3xl border border-stone-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8" @submit.prevent="submit">
    <div class="grid gap-6 sm:grid-cols-2">
      ...
      <label><span class="mb-2 block text-sm font-semibold">Limite</span><input v-model="form.limit_amount" inputmode="decimal" class="w-full rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-950" placeholder="1.000,00" /><p class="mt-2 text-xs text-stone-400">Use vírgula para os centavos.</p><InputError class="mt-2" :message="form.errors.limit_amount" /></label>
      <label><span class="mb-2 block text-sm font-semibold">Período</span><SelectInput v-model="form.period"><option v-for="period in periods" :key="period.value" :value="period.value">{{ period.label }}</option></SelectInput><InputError class="mt-2" :message="form.errors.period" /></label>
      <label><span class="mb-2 block text-sm font-semibold">Início</span><input v-model="form.starts_on" type="date" class="w-full rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-950" /><InputError class="mt-2" :message="form.errors.starts_on" /></label>
