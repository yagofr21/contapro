...
const statusLabel = (status: string) => ({ previewed: 'Aguardando confirmação', confirmed: 'Confirmada', valid: 'Válida', invalid: 'Inválida', duplicate: 'Duplicada', imported: 'Importada' }[status] ?? status);
...
const discard = () => {
    if (props.batch && window.confirm('Descartar esta prévia?')) router.delete(route('imports.destroy', props.batch.id));
};
</script>
...
<template>
  ...
  <AuthenticatedLayout>
    <PageHeader kicker="Entrada assistida" title="Importações CSV" subtitle="Confira cada linha antes de alterar seus dados. Arquivos originais não ficam armazenados.">
      <template #actions>
    ...
    <section class="mt-7 grid gap-4 lg:grid-cols-3">
      <article v-for="(step, index) in [{ title: 'Enviar', text: 'CSV de até 2 MB e 1.000 linhas' }, { title: 'Conferir', text: 'Validação e duplicidades por linha' }, { title: 'Confirmar', text: 'Gravação atômica e auditável' }]" :key="step.title" class="rounded-2xl border p-4" :class="(batch ? index <= 1 : index === 0) ? 'border-brand-200 bg-brand-50 dark:border-brand-900 dark:bg-brand-950/30' : 'border-stone-200 bg-white dark:border-slate-800 dark:bg-slate-900'">
    ...
    <section class="mt-6 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div class="flex items-center gap-3"><span class="grid h-11 w-11 place-items-center rounded-2xl bg-brand-100 text-brand-700 dark:bg-brand-950 dark:text-brand-300"><UploadCloud :size="22" /></span><div><h2 class="font-semibold">Nova importação</h2><p class="text-xs text-stone-500">Separadores ponto e vírgula, vírgula ou tabulação; UTF-8 ou Windows-1252.</p></div></div>
      <form class="mt-5 grid gap-4 lg:grid-cols-[200px_1fr_1.4fr_auto] lg:items-end" @submit.prevent="submitUpload">
        <label><span class="mb-2 block text-xs font-semibold text-stone-500">Conteúdo</span><SelectInput v-model="upload.kind" class="w-full"><option value="financial">Financeiro</option><option value="investment">Investimentos</option></SelectInput><InputError :message="upload.errors.kind" /></label>
        ...
        <button :disabled="upload.processing" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 disabled:opacity-50"><FileSpreadsheet :size="17" />Gerar prévia</button>
      </form>
    </section>
    ...
    <template v-if="batch">
      <section class="mt-6 overflow-hidden rounded-3xl bg-slate-950 text-white shadow-xl">
        <div class="flex flex-col gap-5 border-b border-slate-800 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div><div class="flex flex-wrap items-center gap-2"><h2 class="text-lg font-bold">{{ batch.filename }}</h2><StatusBadge :tone="batch.status === 'confirmed' ? 'emerald' : batch.status === 'previewed' ? 'brand' : 'slate'" :label="statusLabel(batch.status)" /></div><p class="mt-1 text-xs text-slate-400">{{ kindLabel(batch.kind) }}<template v-if="batch.portfolio_name"> · {{ batch.portfolio_name }}</template></p></div>
          <div v-if="batch.status === 'previewed'" class="flex flex-wrap gap-2"><button type="button" class="rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-300" @click="discard">Descartar</button><button :disabled="!canConfirm || confirmation.processing || (batch.summary.invalid > 0 && !confirmation.skip_invalid)" class="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-bold text-slate-950 disabled:opacity-40" @click="confirmImport"><ShieldCheck :size="17" />Confirmar importação</button></div>
        </div>
        <div class="grid grid-cols-2 gap-px bg-slate-800 sm:grid-cols-4"><div v-for="item in [{ label: 'Válidas', value: batch.summary.valid, tone: 'text-emerald-400' }, { label: 'Inválidas', value: batch.summary.invalid, tone: 'text-rose-400' }, { label: 'Duplicadas', value: batch.summary.duplicate, tone: 'text-amber-400' }, { label: 'Importadas', value: batch.summary.imported, tone: 'text-brand-400' }]" :key="item.label" class="min-w-0 bg-slate-950 p-4 sm:p-5"><p class="text-xs text-slate-500">{{ item.label }}</p><p class="mt-1 truncate text-lg font-bold sm:text-2xl" :class="item.tone">{{ item.value }}</p></div></div>
      </section>

      <div v-if="batch.status === 'previewed' && batch.summary.invalid > 0" class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200"><label class="flex cursor-pointer items-start gap-3"><input v-model="confirmation.skip_invalid" type="checkbox" class="mt-0.5 rounded border-amber-300 text-brand-600" /><span><strong>Ignorar {{ batch.summary.invalid }} linha(s) inválida(s)</strong><span class="mt-0.5 block text-xs opacity-80">Somente linhas válidas serão gravadas. Duplicidades sempre são ignoradas.</span></span></label><InputError :message="confirmation.errors.skip_invalid" /></div>

      <Card class="mt-4" title="Conferência por linha" subtitle="Exibindo até 200 das {{ batch.summary.total }} linhas.">
        <template #actions><Check v-if="batch.status === 'confirmed'" class="text-emerald-500" /></template>
        <div class="overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="bg-stone-50 text-xs text-stone-500 dark:bg-slate-950"><tr><th class="px-4 py-3">Linha</th><th v-for="header in headers" :key="header" class="whitespace-nowrap px-4 py-3">{{ header }}</th><th class="px-4 py-3">Situação</th></tr></thead><tbody class="divide-y divide-stone-100 dark:divide-slate-800"><tr v-for="row in rows" :key="row.id"><td class="px-4 py-3 font-mono text-xs text-stone-400">{{ row.row_number }}</td><td v-for="header in headers" :key="header" class="max-w-48 truncate px-4 py-3 sm:max-w-56">{{ row.raw[header] || '—' }}</td><td class="px-4 py-3"><StatusBadge :tone="row.status === 'confirmed' || row.status === 'valid' || row.status === 'imported' ? 'emerald' : row.status === 'invalid' ? 'rose' : row.status === 'duplicate' ? 'amber' : row.status === 'previewed' ? 'brand' : 'slate'" :label="statusLabel(row.status)" /><p v-if="row.errors.length" class="mt-2 text-xs text-rose-600">{{ row.errors.join(' ') }}</p></td></tr></tbody></table></div>
      </Card>
    </template>

    <section v-if="history.length" class="mt-8">
      <div class="flex items-center gap-2"><History :size="18" class="text-stone-400" /><h2 class="font-semibold">Histórico recente</h2></div>
      <div class="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3"><Link v-for="item in history" :key="item.id" :href="route('imports.show', item.id)" class="rounded-2xl border border-stone-200 bg-white p-4 transition hover:border-brand-300 dark:border-slate-800 dark:bg-slate-900"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate text-sm font-semibold">{{ item.filename }}</p><p class="mt-1 text-xs text-stone-500">{{ kindLabel(item.kind) }} · {{ new Intl.DateTimeFormat('pt-BR').format(new Date(item.created_at)) }}</p></div><StatusBadge :tone="item.status === 'confirmed' ? 'emerald' : item.status === 'previewed' ? 'brand' : 'slate'" :label="statusLabel(item.status)" /></div><p class="mt-3 text-xs text-stone-400">{{ item.summary.total }} linhas · {{ item.summary.imported }} importadas</p></Link></div>
    </section>

    <div class="mt-6 flex gap-3 rounded-2xl border border-stone-200 bg-stone-50 p-4 text-xs text-stone-500 dark:border-slate-800 dark:bg-slate-900"><AlertTriangle :size="17" class="shrink-0 text-amber-500" /><p>CSV não executa fórmulas, mas células iniciadas por caracteres de fórmula são neutralizadas nas exportações do Conta Pro. Revise arquivos recebidos de terceiros.</p></div>
  </AuthenticatedLayout>
</template>
