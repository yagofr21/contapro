...
class SyncQuote implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(public int $assetId, public bool $history = true) {}
    ...
    public function handle(MarketDataProvider $provider, PersistPriceHistory $persist): void
    {
        ...
        try {
            $persist->handle($asset, [$provider->quote($asset->symbol)]);
        ...
        if ($this->history) {
            SyncQuoteHistory::dispatch($asset->id);
        }
    }
